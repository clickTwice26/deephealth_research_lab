...
export default function TeamPage() {
    ...
    const fetchTeam = async () => {
        try {
            ...
            const groupings: Record<string, TeamMember[]> = {};
            data.forEach(member => {
                if (!groupings[member.designation]) {
                    groupings[member.designation] = [];
                }
                groupings[member.designation].push(member);
            });

            // Convert to array and calculate max weight for sorting groups
            const groupArray: GroupedMembers[] = Object.entries(groupings).map(([designation, members]) => {
                const maxWeight = Math.max(...members.map(m => m.designation_weight));
                // Sort members within the group by weight
                members.sort((a, b) => b.designation_weight - a.designation_weight);
                return {
                    designation,
                    weight: maxWeight,
                    members
            ...
            // Sort groups by max weight descending
            groupArray.sort((a, b) => b.weight - a.weight);
            ...
            {loading ? (
                ...
                <>
                    {groups.map((group, groupIndex) => (
                        <section key={group.designation} className={`py-16 px-4 sm:px-6 lg:px-8 ${groupIndex % 2 !== 0 ? 'bg-gray-50 dark:bg-gray-900/50' : ''}`}>
                            <div className="max-w-6xl mx-auto">
                                <div className="text-center mb-16">
                                    <h2 className="text-3xl font-bold mb-4">{group.designation}s</h2>
                                    <div className="w-20 h-1 bg-blue-600 mx-auto rounded-full" />
                                </div>

                                {/* Use different layouts based on group importance/index */}
                                {groupIndex === 0 ? (
                                    /* Primary Group (e.g. Leadership) - Detailed Layout */
                                        ...
                                        {group.members.map((person, index) => (
                                            ...
                                            >
                                                <div className="relative group shrink-0">
                                                    <div className="absolute inset-0 bg-blue-600 rounded-2xl blur opacity-20 group-hover:opacity-40 transition-opacity" />
                                                    <img
                                                        src={getAvatar(person.name, person.profile_image)}
                                                        alt={person.name}
                                                        className="w-48 h-64 rounded-2xl object-cover border-4 border-white dark:border-gray-800 relative z-10 shadow-lg"
                                                    />
                                                </div>
                                                <div>
                                                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{person.name}</h3>
                                                    <p className="text-blue-600 dark:text-blue-400 font-medium">{person.designation}</p>
                                                    {person.university && <p className="text-gray-500 text-sm mb-3">{person.university}</p>}
                                                    {/* Email Display */}
                                                    {person.email && (
                                                        <a href={`mailto:${person.email}`} className="block text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 text-sm mb-3 transition-colors">
                                            ...
                                            >
                                                <div className="mb-4 inline-block relative">
                                                    <img
                                                        src={getAvatar(person.name, person.profile_image)}
                                                        alt={person.name}
                                                        className="w-40 h-52 rounded-2xl object-cover mb-4 mx-auto group-hover:scale-105 transition-transform duration-300"
                                                    />
                                                </div>
                                                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1">{person.name}</h3>
                                                {/* Designation is redundant if header is designation, but kept for clarity */}
                                                {/* <p className="text-sm text-blue-600 dark:text-blue-400 font-medium mb-4">{person.designation}</p> */}
                                                {person.university && <p className="text-sm text-gray-500 mb-2">{person.university}</p>}
                                                {person.email && (
                                                    <a href={`mailto:${person.email}`} className="block text-xs text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 mb-4 transition-colors">
                                                        {person.email}
                                                    </a>
                                                )}
